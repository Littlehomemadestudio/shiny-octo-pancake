#!/usr/bin/env python3
"""
Enhanced Quran Reader App - Complete Version
A comprehensive desktop application for daily Quran reading with advanced features.
...
class QuranPageManager:
    """Manages Quran page images and display"""
    
    def __init__(self, quran_pages_dir: str = r"C:\Users\Littl\Downloads\quranpages\114"):
        self.quran_pages_dir = Path(quran_pages_dir)
        self.page_images = {}
        self.current_image = None
        self.image_cache = {}
        self.max_cache_size = 10  # Cache up to 10 images
        
    def get_page_image_path(self, page_number: int) -> Optional[Path]:
        """Get the path to a Quran page image"""
        # Try different possible naming patterns
        possible_names = [
            f"page_{page_number:03d}.jpg",
            f"page_{page_number:03d}.png",
            f"page_{page_number:03d}.jpeg",
            f"{page_number:03d}.jpg",
            f"{page_number:03d}.png",
            f"{page_number:03d}.jpeg",
            f"page{page_number}.jpg",
            f"page{page_number}.png",
            f"page{page_number}.jpeg",
            f"{page_number}.jpg",
            f"{page_number}.png",
            f"{page_number}.jpeg"
        ]
        ...
        for name in possible_names:
            image_path = self.quran_pages_dir / name
            if image_path.exists():
                return image_path
        
        return None
    
    def load_page_image(self, page_number: int, max_width: int = 800, max_height: int = 600) -> Optional[ImageTk.PhotoImage]:
        """Load and resize a Quran page image"""
        # Check cache first
        ...
        if not image_path or not image_path.exists():
            logging.warning(f"Quran page image not found for page {page_number}")
            return None
        ...
        try:
            ...
            self.image_cache[page_number] = photo
            return photo
            
        except Exception as e:
            logging.error(f"Error loading image for page {page_number}: {e}")
            return None
    
    def create_placeholder_image(self, page_number: int, max_width: int = 800, max_height: int = 600) -> ImageTk.PhotoImage:
        """Create a placeholder image when Quran page is not available"""
        try:
            # Create a placeholder image
            placeholder = Image.new('RGB', (max_width, max_height), color='#f5f5f5')
            ...
            text_large = f"Page {page_number}"
            text_medium = "Quran Page"
            text_small = f"Page {page_number} of 604"
            
            # Get text dimensions for centering
            bbox_large = draw.textbbox((0, 0), text_large, font=font_large)
            bbox_medium = draw.textbbox((0, 0), text_medium, font=font_medium)
            bbox_small = draw.textbbox((0, 0), text_small, font=font_small)
            
            text_width_large = bbox_large[2] - bbox_large[0]
            text_width_medium = bbox_medium[2] - bbox_medium[0]
            text_width_small = bbox_small[2] - bbox_small[0]
            
            # Draw centered text
            draw.text(((max_width - text_width_large) // 2, max_height // 2 - 60), 
                     text_large, fill='#2e7d32', font=font_large)
            draw.text(((max_width - text_width_medium) // 2, max_height // 2 - 20), 
                     text_medium, fill='#4caf50', font=font_medium)
            draw.text(((max_width - text_width_small) // 2, max_height // 2 + 20), 
                     text_small, fill='#666666', font=font_small)
            
            # Add decorative border
            draw.rectangle([10, 10, max_width-10, max_height-10], outline='#2e7d32', width=3)
            
            return ImageTk.PhotoImage(placeholder)
            
        except Exception as e:
            logging.error(f"Error creating placeholder for page {page_number}: {e}")
            # Return a simple colored rectangle as fallback
            placeholder = Image.new('RGB', (max_width, max_height), color='#f5f5f5')
            return ImageTk.PhotoImage(placeholder)
    
    def get_page_display_image(self, page_number: int, max_width: int = 800, max_height: int = 600) -> ImageTk.PhotoImage:
        """Get image for display, with fallback to placeholder"""
        # Try to load actual Quran page image
        image = self.load_page_image(page_number, max_width, max_height)
        
        if image is not None:
            return image
        
        # Fallback to placeholder
        logging.info(f"Using placeholder for page {page_number}")
        return self.create_placeholder_image(page_number, max_width, max_height)
...
class QuranApp:
    def __init__(self, root):
        ...
        self.quran_page_manager = QuranPageManager()
    ...
    def load_settings(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'quran_pages_dir': r"C:\Users\Littl\Downloads\quranpages\114"
            }
    ...
    def create_main_content(self, parent):
        ...
        # Image label for Quran page
        self.page_image_label = tk.Label(content_frame, bg='#f5f5f5')
        self.page_image_label.pack(expand=True, fill='both')
    ...
    def create_reading_controls(self, parent):
        ...
        self.prev_button = tk.Button(button_frame, text="Previous Page", 
                                    font=('Arial', 14, 'bold'), bg='#ff9800', fg='white',
                                    command=self.previous_page, width=18, height=2)
        ...
        self.stats_button = tk.Button(button_frame, text="Statistics", 
                                     font=('Arial', 14, 'bold'), bg='#607d8b', fg='white',
    ...
    def update_timer(self):
        ...
        if self.timer_running:
            ...
            # Enable next page button after 5 minutes (300 seconds)
    ...
    def previous_page(self):
        ...
        if self.current_page > 1:
            self.current_page -= 1
            self.update_display()
            logging.info(f"Went back to page {self.current_page}")
        else:
    ...
    def open_settings(self):
        ...
        # Quran pages directory setting
        dir_frame = tk.Frame(settings_window, bg='#f0f8ff')
        dir_frame.pack(fill='x', padx=20, pady=10)
        
        tk.Label(dir_frame, text="Quran Pages Directory:", font=('Arial', 12), 
                bg='#f0f8ff').pack(anchor='w')
        
        dir_entry = tk.Entry(dir_frame, font=('Arial', 10))
        dir_entry.pack(fill='x', pady=(5, 0))
        dir_entry.insert(0, self.settings.get('quran_pages_dir', ''))
        ...
        def save_settings():
            ...
            self.settings['quran_pages_dir'] = dir_entry.get()
            
            # Update Quran page manager with new directory
            self.quran_page_manager = QuranPageManager(dir_entry.get())
    ...
    def update_display(self):
        ...
        # Load and display Quran page image
        try:
            page_image = self.quran_page_manager.get_page_display_image(self.current_page)
            self.page_image_label.config(image=page_image)
            self.page_image_label.image = page_image  # Keep a reference
        except Exception as e:
            logging.error(f"Error loading page image: {e}")
            # Fallback to text display
            self.page_image_label.config(text=f"Page {self.current_page} of the Holy Quran\n\n[Image loading error - check directory path]")
