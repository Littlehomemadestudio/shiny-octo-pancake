...
# -------------------- CONFIGURATION --------------------
TOKEN = "YOUR_BALE_BOT_TOKEN"  # Replace with your Bale bot token
...
MILITARY_ASSETS = {
    # Tanks
    "m1_abrams": {"cost": 50, "power": 5, "name": "M1 Abrams Tank"},
    "t90": {"cost": 100, "power": 10, "name": "T-90 Tank"},
    "leopard2": {"cost": 200, "power": 20, "name": "Leopard 2 Tank"},
    "challenger2": {"cost": 500, "power": 50, "name": "Challenger 2 Tank"},
    
    # Aircraft
    "f16": {"cost": 100, "power": 15, "name": "F-16 Fighting Falcon"},
    "f22": {"cost": 250, "power": 35, "name": "F-22 Raptor"},
    "su35": {"cost": 500, "power": 70, "name": "Su-35 Flanker"},
    "predator_drone": {"cost": 150, "power": 25, "name": "MQ-1 Predator Drone"},
    
    # Missiles
    "tomahawk": {"cost": 300, "power": 60, "name": "Tomahawk Missile"},
    "patriot": {"cost": 1000, "power": 200, "name": "Patriot Missile System"},
    "javelin": {"cost": 800, "power": 150, "name": "Javelin Anti-Tank Missile"},
    
    # Navy
    "destroyer": {"cost": 1200, "power": 250, "name": "Arleigh Burke Destroyer"},
    "submarine": {"cost": 1500, "power": 300, "name": "Virginia Class Submarine"},
    "aircraft_carrier": {"cost": 5000, "power": 1000, "name": "Nimitz Class Carrier"},
    
    # Infantry
    "soldier": {"cost": 10, "power": 1, "name": "Infantry Soldier"},
    "special_forces": {"cost": 100, "power": 15, "name": "Special Forces Unit"},
    
    # Artillery
    "howitzer": {"cost": 300, "power": 55, "name": "M777 Howitzer"},
    "mlrs": {"cost": 400, "power": 70, "name": "M270 MLRS"},
    
    # Defense Systems
    "iron_dome": {"cost": 350, "power": 60, "name": "Iron Dome Defense"},
    "radar": {"cost": 200, "power": 10, "name": "AN/SPY-1 Radar"},
}
...
async def handle_command(message, command, chat_id, user_id):
    ...
    try:
        # General commands
        if command == "/start":
            await message.reply("🤖 Group Management Bot is active!\n\n"
                              "Available commands:\n"
                              "/help - Help\n"
                              "/rules - Show rules\n"
                              "/info - User info\n"
        ...
        elif command == "/clean" and is_owner(user_id, chat_id):
            await clean_command(message, chat_id)
        
        else:
            await message.reply("🚫 Invalid command or no permission!")
    except Exception as e:
        print(f"Error in handle_command: {e}")
        print(traceback.format_exc())
        await message.reply("⚠️ Error processing command!")
...
async def show_points(message, chat_id, user_id):
    ...
    try:
        user_data = get_user_data(chat_id, user_id)
        await message.reply(f"💰 Your points: {user_data['points']}")
        await delete_message_safe(message, 10)  # Delete after 10 seconds
    except Exception as e:
        print(f"Error in show_points: {e}")
        await message.reply("⚠️ Error showing points!")
...
async def show_military(message, chat_id, user_id):
    ...
    try:
        ...
        await message.reply("⚠️ Error showing military force!")
...
async def buy_asset(message, chat_id, user_id):
    ...
    try:
        ...
        if len(parts) < 3:
            await message.reply("⚠️ Wrong buy command format. Use: /buy [type] [quantity]")
            return
        ...
        try:
            ...
            if quantity <= 0:
                await message.reply("⚠️ Quantity must be greater than zero!")
                return
        except ValueError:
            await message.reply("⚠️ Quantity must be a number!")
            return
        
        if asset_type not in MILITARY_ASSETS:
            await message.reply("⚠️ Invalid equipment type! Use /game_help to see valid types.")
            return
        ...
        if user_data["points"] < total_cost:
            await message.reply(f"⚠️ Not enough points! Need: {total_cost} points (You have: {user_data['points']})")
            return
        ...
        await message.reply("⚠️ Error buying equipment!")

async def attack_user(message, chat_id, user_id):
    """Attack another user"""
    try:
        if not message.reply_to_message:
            await message.reply("⚠️ Please reply to the user you want to attack!")
            return
        
        target_user = message.reply_to_message.author
        if target_user.user_id == user_id:
            await message.reply("🤣⚠️ You can't attack yourself!")
            return
        ...
        if attacker_power == 0:
            await message.reply("⚠️ You have no military force to attack!")
            return
        
        # Check if defender has military force
        defender_power = calculate_total_power(defender_data)
        if defender_power == 0:
            await message.reply("⚠️ Target user has no military force to defend!")
            return
        ...
        if attack_strength > defense_strength:
            ...
            result_text = (f"⚔️ {message.author.first_name} attacked {target_user.first_name} and won!\n"
            ...
            result_text = (f"🛡 {message.author.first_name} attacked {target_user.first_name} but lost!\n"
        ...
        await message.reply("⚠️ Error in attack!")
...
async def handle_alliance_command(message, chat_id, user_id):
    ...
    parts = message.content.split()
    if len(parts) < 2:
        await message.reply("⚠️ Wrong command format. Use /game_help for guidance.")
        return
    ...
    if subcommand == "create" and len(parts) >= 3:
        ...
        await message.reply("⚠️ Invalid alliance command. Use /game_help for guidance.")
...
async def alliance_create(message, chat_id, user_id, alliance_name):
    ...
    try:
        ...
        if user_data["alliance"]:
            await message.reply("⚠️ You are already in an alliance. Leave your current alliance first.")
            return
        
        if alliance_name in chat_data["alliances"]:
            await message.reply("⚠️ An alliance with this name already exists.")
            return
        ...
        save_data()
        await message.reply(f"✅ Alliance '{alliance_name}' created successfully and you joined it!")
        
    except Exception as e:
        print(f"Error in alliance_create: {e}")
        await message.reply("⚠️ Error creating alliance!")
...
async def alliance_join(message, chat_id, user_id, alliance_name):
    ...
    try:
        ...
        if alliance_name not in chat_data["alliances"]:
            await message.reply("⚠️ No alliance with this name exists.")
            return
        ...
        save_data()
        await message.reply(f"✅ You successfully joined alliance '{alliance_name}'!")
        
    except Exception as e:
        print(f"Error in alliance_join: {e}")
        await message.reply("⚠️ Error joining alliance!")
...
async def alliance_leave(message, chat_id, user_id):
    ...
    try:
        ...
        if not user_data["alliance"]:
            await message.reply("⚠️ You are not in any alliance.")
            return
        ...
        if alliance_name not in chat_data["alliances"]:
            ...
            save_data()
            await message.reply("⚠️ Your alliance doesn't exist. Your status has been updated.")
            return
        ...
        save_data()
        await message.reply(f"✅ You successfully left alliance '{alliance_name}'!")
        
    except Exception as e:
        print(f"Error in alliance_leave: {e}")
        await message.reply("⚠️ Error leaving alliance!")
...
async def alliance_info(message, chat_id, alliance_name):
    ...
    try:
        ...
        info_text = (
            f"🤝 Alliance '{alliance_name}' Information:\n\n"
        ...
        await message.reply("⚠️ Error getting alliance information!")
...
async def alliance_list(message, chat_id):
    ...
    try:
        ...
        if not chat_data["alliances"]:
            await message.reply("⚠️ No alliances exist in this group.")
            return
        
        alliances_text = "🤝 Group Alliances List:\n\n"
        ...
        await message.reply("⚠️ Error getting alliances list!")

async def alliance_invite(message, chat_id, user_id):
    """Invite a user to your alliance"""
    try:
        if not message.reply_to_message:
            await message.reply("⚠️ Please reply to the user you want to invite!")
            return
        ...
        if target_user_data["alliance"]:
            await message.reply("⚠️ This user is already in an alliance.")
            return
        ...
        if user_id != alliance["creator"]:
            await message.reply("⚠️ Only the alliance creator can invite new users.")
            return
        ...
        await message.reply("⚠️ Error sending invitation!")

async def alliance_kick(message, chat_id, user_id):
    """Kick a user from your alliance"""
    try:
        if not message.reply_to_message:
            await message.reply("⚠️ Please reply to the user you want to kick!")
            return
        ...
        if alliance_name != target_user_data["alliance"]:
            await message.reply("⚠️ This user is not in your alliance.")
            return
        ...
        # Check if user is the creator or has permission
        if user_id != alliance["creator"]:
            await message.reply("⚠️ Only the alliance creator can kick users.")
            return
        
        # Cannot kick yourself
        if target_user_id == user_id:
            await message.reply("⚠️ You can't kick yourself.")
            return
        ...
        save_data()
        await message.reply(f"✅ User {target_user.first_name} has been kicked from the alliance!")
        
    except Exception as e:
        print(f"Error in alliance_kick: {e}")
        await message.reply("⚠️ Error kicking user!")

# -------------------- CLEAN COMMAND --------------------
async def clean_command(message, chat_id):
    ...
    try:
        ...
        clean_msg = await message.reply("🧹 Clean command executed! Bot will delete its messages automatically.")
        
        # Delete the confirmation message after 5 seconds
        await delete_message_safe(clean_msg, 5)
        
    except Exception as e:
        print(f"Error in clean_command: {e}")
        await message.reply("⚠️ Error executing clean command!")
...
async def show_rules(message, chat_id):
    ...
    try:
        ...
        await message.reply("⚠️ Error showing rules!")
...
async def user_info(message):
    ...
    try:
        ...
        await message.reply(f"👤 User Information:\n\n"
        ...
        await message.reply("⚠️ Error showing user information!")

async def set_owner(message, chat_id):
    """Set group owner"""
    try:
        if not await is_creator(message.author.user_id, chat_id):
            await message.reply("🚫 Only group creator can set owner!")
            return
            
        if not message.reply_to_message:
            await message.reply("⚠️ Please reply to the user you want to set as owner!")
            return
        ...
        await message.reply(f"✅ Group owner set successfully!")
        
    except Exception as e:
        print(f"Error in set_owner: {e}")
        await message.reply("⚠️ Error setting owner!")
...
async def mute_user(message, chat_id):
    ...
    try:
        ...
        parts = message.content.split()
        if len(parts) < 2:
            await message.reply("⚠️ Command format: /mute [time] [reason]\nExample: /mute 1h spam")
            return
        ...
        if not message.reply_to_message:
            await message.reply("⚠️ Please reply to the user you want to mute!")
            return
        ...
        await message.reply(f"🔇 User muted for {format_time(duration)}.\nReason: {reason}")
...
async def unmute_user(message, chat_id):
    ...
    try:
        if not message.reply_to_message:
            await message.reply("⚠️ Please reply to the user you want to unmute!")
            return
        ...
        if str(target_id) in chat_data["muted_users"]:
            ...
            await message.reply("⚠️ User is not muted!")
    except Exception as e:
        print(f"Error in unmute_user: {e}")
        await message.reply("⚠️ Error unmuting user!")

async def ban_user(message, chat_id):
    """Ban user from group"""
    try:
        if not message.reply_to_message:
            await message.reply("⚠️ Please reply to the user you want to ban!")
            return
        ...
        await message.reply(f"🚫 User banned from group.\nReason: {reason}")
        
    except Exception as e:
        print(f"Error in ban_user: {e}")
        await message.reply("⚠️ Error banning user!")

async def warn_user(message, chat_id):
    """Warn user"""
    try:
        if not message.reply_to_message:
            await message.reply("⚠️ Please reply to the user you want to warn!")
            return
        ...
        await message.reply(f"⚠️ User warned.\nReason: {reason}\nWarning count: {warning_count}")
        
    except Exception as e:
        print(f"Error in warn_user: {e}")
        await message.reply("⚠️ Error warning user!")
...
async def set_rules(message, chat_id):
    ...
    try:
        ...
        if not new_rules:
            await message.reply("⚠️ Please enter the rules text!")
            return
        ...
        await message.reply("✅ Group rules updated successfully!")
        
    except Exception as e:
        print(f"Error in set_rules: {e}")
        await message.reply("⚠️ Error saving rules!")
...
async def set_welcome(message, chat_id):
    ...
    try:
        ...
        if not new_welcome:
            await message.reply("⚠️ Please enter the welcome message text!")
            return
        ...
        await message.reply("⚠️ Error saving welcome message!")
...
async def add_admin(message, chat_id):
    ...
    try:
        parts = message.content.split()
        if len(parts) < 2:
            await message.reply("⚠️ Command format: /addadmin [user_id]")
            return
        ...
        if new_admin_id not in chat_data["admins"]:
            ...
            save_data()
            await message.reply(f"✅ User {new_admin_id} added to admin list!")
        else:
        ...
        await message.reply("⚠️ Error adding admin!")
...
async def remove_admin(message, chat_id):
    ...
    try:
        parts = message.content.split()
        if len(parts) < 2:
            await message.reply("⚠️ Command format: /removeadmin [user_id]")
            return
        ...
        if admin_id in chat_data["admins"]:
            ...
            save_data()
            await message.reply(f"✅ User {admin_id} removed from admin list!")
        else:
            await message.reply("⚠️ User is not in admin list!")
            
    except ValueError:
        await message.reply("⚠️ User ID must be a number!")
    except Exception as e:
        print(f"Error in remove_admin: {e}")
        await message.reply("⚠️ Error removing admin!")
...
async def show_owner(message, chat_id):
    ...
    try:
        ...
        if chat_data["owner_id"]:
            await message.reply(f"👑 Group Owner: {chat_data['owner_id']}")
        else:
            await message.reply("⚠️ Group owner not set. Use /setowner to set one.")
    except Exception as e:
        print(f"Error in show_owner: {e}")
        await message.reply("⚠️ Error showing group owner!")
...
async def on_chat_leave(chat_member: ChatMember):
    ...
    try:
        ...
        await bot.send_message(chat_id, f"👋 {user.first_name} left the group.")
    except Exception as e:
