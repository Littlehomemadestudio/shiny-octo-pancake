...
# -------------------- CONFIGURATION --------------------
TOKEN = "YOUR_BALE_BOT_TOKEN"  # Replace with your actual token
...
async def handle_command(message, command, chat_id, user_id):
    ...
    try:
        ...
        # Owner commands
            ...
            await message.reply("🚫 Invalid command or no access!")
    except Exception as e:
...
async def clean_command(message, chat_id, user_id):
    ...
    try:
        ...
        if not is_owner(user_id, chat_id):
            await message.reply("🚫 Only group owner can use this command!")
            return
        ...
        confirmation = await message.reply("🧹 Cleaning bot messages...")
        await delete_message_safe(confirmation, 3)
        
    except Exception as e:
        print(f"Error in clean_command: {e}")
        await message.reply("⚠️ Error cleaning messages!")
...
async def buy_asset(message, chat_id, user_id):
    ...
    try:
        ...
        if len(parts) < 3:
            await message.reply("⚠️ Invalid buy command format")
            return
        ...
        if asset_type not in MILITARY_ASSETS:
            await message.reply("⚠️ Invalid equipment type! Use /game_help for valid types.")
            return
        ...
        if user_data["points"] < total_cost:
            await message.reply(f"⚠️ Insufficient points! Need: {total_cost} points (Have: {user_data['points']})")
            return
...
async def handle_alliance_command(message, chat_id, user_id):
    ...
    parts = message.content.split()
    if len(parts) < 2:
        await message.reply("⚠️ Invalid command format. Use /game_help for guidance.")
        return
...
async def alliance_create(message, chat_id, user_id, alliance_name):
    ...
    try:
        ...
        if user_data["alliance"]:
            await message.reply("⚠️ You are already in an alliance. Leave current one first.")
            return
...
async def alliance_leave(message, chat_id, user_id):
    ...
    try:
        ...
        if alliance_name not in chat_data["alliances"]:
            ...
            save_data()
            await message.reply("⚠️ Your alliance doesn't exist. Status updated.")
            return
...
async def alliance_info(message, chat_id, alliance_name):
    ...
    try:
        ...
        await message.reply("⚠️ Error getting alliance info!")
...
async def alliance_list(message, chat_id):
    ...
    try:
        ...
        await message.reply("⚠️ Error getting alliance list!")
...
async def alliance_invite(message, chat_id, user_id):
    ...
    try:
        ...
        if user_id != alliance["creator"]:
            await message.reply("⚠️ Only alliance creator can invite new members.")
            return
...
async def alliance_kick(message, chat_id, user_id):
    ...
    try:
        ...
        # Check if user is the creator or has permission
        if user_id != alliance["creator"]:
            await message.reply("⚠️ Only alliance creator can kick members.")
            return
        ...
        save_data()
        await message.reply(f"✅ User {target_user.first_name} kicked from alliance!")
...
async def user_info(message):
    ...
    try:
        ...
        await message.reply("⚠️ Error showing user info!")
